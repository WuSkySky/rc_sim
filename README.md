...
- 重定位后前往第一个放置点时使用 `1.125 m/s` 线速度上限，即位置伺服默认
  `0.675 m/s` 上限的 `5/3` 倍；后续移动不覆盖默认速度。
...
位置伺服默认 X/Y 线速度上限为 `0.675 m/s`。下台阶动作的首个 `b1` 请求使用
`step_traverse.yaml` 中独立的 `b1_linear_speed_limit`，默认 `0.27 m/s`，即默认
线速度的一半再乘 `0.8`；`b2`、`b3` 继续使用位置伺服默认上限。
