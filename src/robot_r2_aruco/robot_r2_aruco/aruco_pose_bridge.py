    ...
    def _on_parameters_changed(self, parameters) -> SetParametersResult:
        for parameter in parameters:
            if parameter.name == "target_marker_id":
                ...
                self._target_marker_id = int(value)
                self.get_logger().info(
                    f"target marker changed to {int(value)}"
                )
                ...
                self._publish_rate = value
                self._timer.cancel()
                self._timer = self.create_timer(1.0 / value, self._publish)
                self.get_logger().info(f"publish rate changed to {value:g} Hz")
