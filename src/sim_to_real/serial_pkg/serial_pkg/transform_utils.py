"""Minimal quaternion/transform helpers shared by serial_pkg TF nodes.

This is a self-contained copy of the generic transform math used by
``odin_data_postprocess``. serial_pkg keeps its own copy so the lower-machine
TF node does not depend on the Odin package it replaces.
"""
